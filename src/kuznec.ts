...
export class Kuznec{
    ...
    calculateAvalancheEffect(key: Buffer, plaintext: Uint8Array): number {
        const originalCipherText = encryptBlock(key, plaintext);
        let differentBitsCount = 0;

        for (let i = 0; i < plaintext.length; i++) {
            let modifiedPlaintext = plaintext.slice(); // Create a copy of the original plaintext
            modifiedPlaintext[i] ^= 1; // Change one bit

            const modifiedCipherText = encryptBlock(key, modifiedPlaintext);

            for (let j = 0; j < originalCipherText.length; j++) {
                if (originalCipherText[j] !== modifiedCipherText[j]) {
                    differentBitsCount++;
                }
            }
        }
